...
from . import mhc_split as _prefill_split

_PREFILL_SPLIT_ENABLED = os.environ.get("VLLM_FL_METAX_MHC_PREFILL_SPLIT", "0") == "1"
try:
    from . import mhc_mm as _mhc_pre_mm_op  # noqa: F401
except Exception as _mhc_pre_mm_op_exc:
    _MHC_PRE_MM_OP_IMPORT_ERROR = _mhc_pre_mm_op_exc
else:
    _MHC_PRE_MM_OP_IMPORT_ERROR = None
from .identity_cache import IdentityCache

_FN_BF16_CACHE = IdentityCache()
...
@triton.autotune(
    configs=[triton.Config({"BLOCK_H": 8192}, num_warps=4, num_stages=1)],
...
def mhc_pre(
    ...
    if _MHC_PRE_MM_OP_IMPORT_ERROR is not None:
        raise RuntimeError(
            f"mhc_pre_mm_op import failed: {_MHC_PRE_MM_OP_IMPORT_ERROR!r}"
        )
    gemm_out = torch.ops.vllm_fl.dsv4_mhc_pre_mm(x_flat, fn_bf16).float()
    if _PREFILL_SPLIT_ENABLED and hc_mult == 4:
        (post_mix, comb_mix, layer_input) = _prefill_split.tail(
            gemm_out,
            hc_scale,
            hc_base,
            residual_flat,
