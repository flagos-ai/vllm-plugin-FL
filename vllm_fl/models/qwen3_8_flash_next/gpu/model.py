...
class Qwen3_8FlashNextForConditionalGeneration(
    ...
    if not hasattr(Qwen3_5ForConditionalGeneration, "_init_video_pruning"):
        # vLLM 0.24 has no _init_video_pruning anywhere in the tree; 0.26 grew
        # one on Qwen3VLForConditionalGeneration (qwen3_vl.py) and we inherit
        # it there. Define the 0.24 fallback only when the base class lacks
        # it, so 0.26 keeps using upstream's version rather than this stub.
