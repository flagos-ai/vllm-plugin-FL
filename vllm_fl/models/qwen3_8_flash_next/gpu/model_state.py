...
class Qwen3_8FlashNextModelState(MambaHybridModelState):
    ...
    def _prepare_ngram_context(
        ...
        token_store = req_states.all_token_ids.gpu
        valid_tokens = (token_indices >= 0) & (
            token_indices < token_store.shape[1]
        )
        # The zero index is a gather sentinel for graph padding only.  It is
        # never used to turn a real out-of-range context position into valid
        # model input; the corresponding output is replaced with EOS below.
        safe_token_indices = torch.where(
            valid_tokens, token_indices, torch.zeros_like(token_indices)
        )
        context_tokens = token_store[
            request_indices.unsqueeze(1), safe_token_indices
        ]
