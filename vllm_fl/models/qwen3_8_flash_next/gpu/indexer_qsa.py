...
class QSAIndexer(nn.Module):
    ...
    def _select(
        ...
        return qsa_select_paged_tokens(
            ...
            self.compress_ratio,
            out,
            # The block table addresses whole KV-manager blocks, each holding
            # storage_block_size compressed rows. vLLM may allocate the cache
            # as several smaller kernel pages per block, so the reader must be
            # told the block unit explicitly -- it cannot infer it from the
            # cache shape. This is the same value the writer encodes slots
            # with (see compressed_qsa_slot_mapping).
            storage_block_size=metadata.storage_block_size,
        )
