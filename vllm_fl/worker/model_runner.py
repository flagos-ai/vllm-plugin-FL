...
class ModelRunnerFL(LoRAModelRunnerMixin, KVConnectorModelRunnerMixin):
    ...
    def __init__(
        ...
        self.prepare_inputs_event: Optional[current_platform.torch_device_fn.Event] = (
            None
        )
        if self.use_async_scheduling:
            ...
            self.prepare_inputs_event.record(
                current_platform.torch_device_fn.default_stream()
            )
        ...
        if force_attention or cudagraph_runtime_mode == CUDAGraphMode.FULL:
            ...
            for kv_cache_group_id, kv_cache_group_spec in enumerate(
                ...
                for attn_group in self.attn_groups[kv_cache_group_id]:
                    ...
                    attn_metadata_i = (
                        attn_group.get_metadata_builder().build_for_cudagraph_capture(
                            common_attn_metadata
                        )
                    )
                    for layer_name in attn_group.layer_names:
        ...
        for layer_name, attn_module in attn_layers.items():
            ...
            if attn_module.attn_type == AttentionType.DECODER:
                if attn_module.sliding_window is not None:
                    assert not use_mla, "MLA is not supported for sliding" "window"
