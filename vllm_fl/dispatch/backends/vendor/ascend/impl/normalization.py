...
from .fused_moe.utils import get_weight_prefetch_method
from .layernorm_gated import layer_norm_fwd_npu


def enable_custom_op() -> bool:
    """Keep migrated normalization code importable inside vllm_fl."""
    return False
...
def rms_norm_ascend(
    ...
    """Dispatch entry used by the Ascend backend registry."""
    import torch_npu

    if residual is not None:
        x, _, residual = torch_npu.npu_add_rms_norm(
            x, residual, obj.weight, obj.variance_epsilon
        )
        return x, residual

    x, _ = torch_npu.npu_rms_norm(x, obj.weight, obj.variance_epsilon)
    return x
...
class AscendGemmaRMSNorm(GemmaRMSNorm):
    def forward_oot(
        ...
        if hasattr(torch_npu, "npu_gemma_rms_norm"):
            x = torch_npu.npu_gemma_rms_norm(
