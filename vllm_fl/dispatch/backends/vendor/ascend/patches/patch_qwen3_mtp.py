...
def patch_qwen3_next_mtp() -> None:
    ...
    def _ascend_qwen3_next_mtp_load_weights(
        ...
        For MoE checkpoints the routed expert weights are stored as packed
        tensors (``experts.gate_up_proj`` / ``experts.down_proj``) while the
        upstream ``Qwen3NextMultiTokenPredictor`` loader expects per-expert
        slices (``experts.{eid}.gate_proj`` / ``up_proj`` / ``down_proj``) so
        that ``FusedMoE.make_expert_params_mapping`` can route them to the
        packed ``w13_weight`` / ``w2_weight`` model parameters.  We split the
        checkpoint tensors into 2-D per-expert slices before delegating.
        """

        # Only the MTP drafter weights carry the ``mtp.`` prefix; target-model
        # MoE weights live under ``model.language_model.*`` and are consumed by
        # the target-model loader, so we must not rename them here.
        _MTP_PACKED_EXPERT_RE = re.compile(
            r"^mtp\.layers\.(\d+)\.mlp\.experts\.(gate_up_proj|down_proj)$"
        )
        ...
        def _unpack_mtp_moe_packed_weights(weights):
            """Split packed MoE expert tensors into 2-D per-expert slices.

            Qwen3.5/Qwen3.6 MoE checkpoints store routed experts as
            ``experts.gate_up_proj`` (shape ``(E, 2*I, H)``) and
            ``experts.down_proj`` (shape ``(E, H, I)``).  The upstream
            ``Qwen3NextMultiTokenPredictor`` loader expects per-expert
            ``gate_proj``/``up_proj``/``down_proj`` slices, which
            ``FusedMoE.make_expert_params_mapping`` routes to the packed
            ``w13_weight``/``w2_weight`` model parameters.
            """
            # Materialise the iterator: we need to inspect down_proj shapes
            # before we can safely split gate_up_proj.
            weight_list = list(weights)

            # Map layer index -> intermediate_size (I) inferred from down_proj.
            layer_to_intermediate_size: dict[int, int] = {}
            for name, tensor in weight_list:
                match = _MTP_PACKED_EXPERT_RE.match(name)
                if match is None:
                    continue
                proj_type = match.group(2)
                if proj_type != "down_proj" or tensor.ndim != 3:
                    continue
                # down_proj shape is (E, H, I); the smallest non-expert dim is I.
                layer_idx = int(match.group(1))
                layer_to_intermediate_size[layer_idx] = min(
                    tensor.shape[1], tensor.shape[2]
                )

            for name, tensor in weight_list:
                match = _MTP_PACKED_EXPERT_RE.match(name)
                if match is None or tensor.ndim != 3:
                    yield name, tensor
                    continue
                ...
                if proj_type == "gate_up_proj":
                    intermediate_size = layer_to_intermediate_size.get(layer_idx)
                    if intermediate_size is None:
                        logger.warning(
                            "No down_proj found for layer %s; cannot split "
                            "gate_up_proj, passing through unchanged",
                            layer_idx,
                        )
                        yield name, tensor
                        continue

                    packed_size = 2 * intermediate_size
                    packed_dim = None
                    ...
                    if packed_dim is None:
                        logger.warning(
                            "Unable to find packed dim (expected size %s) "
                            "for %s with shape %s; passing through unchanged",
                            packed_size,
                            name,
                            tensor.shape,
                        )
                    ...
                    gate_tensor, up_tensor = tensor.chunk(2, dim=packed_dim)
                    for expert_id in range(num_experts):
                        yield (
                            f"mtp.layers.{layer_idx}.mlp.experts."
                            f"{expert_id}.gate_proj.weight",
                            gate_tensor[expert_id],
                        )
                        yield (
                            f"mtp.layers.{layer_idx}.mlp.experts."
        ...
        return _orig_load_weights(
            self,
            _unpack_mtp_moe_packed_weights(
                _remap_language_model_prefix(weights)
            ),
        )
    ...
    def _ascend_qwen3_next_mtp_init(
        ...
        if cfg.model_type in ("qwen3_5", "qwen3_5_moe"):
            cfg = _rewrite_qwen3_5_config_for_mtp(copy.deepcopy(cfg))
...
def _rewrite_qwen3_5_config_for_mtp(hf_config):
    """Flatten a qwen3.5/qwen3.5_moe config and return a ``Qwen3NextConfig``.

    Upstream ``Qwen3NextMTP`` / ``Qwen3NextMultiTokenPredictor`` instantiate
    ``Qwen3NextDecoderLayer``, which expects a real ``Qwen3NextConfig`` object
    (it reads ``decoder_sparse_step``, ``mlp_only_layers`` and uses the default
    ``layer_types`` generator).  Simply mutating the model_type on the original
    ``Qwen3_5MoeConfig`` is not enough for MoE checkpoints, so we build a fresh
    ``Qwen3NextConfig`` from the flattened text fields.
    """
    ...
    # Fields accepted by Qwen3NextConfig.__init__.  We copy whatever exists on
    # the original config and supply sane defaults for the rest.
    qwen3_next_fields = {
        "vocab_size",
        "hidden_size",
        "intermediate_size",
    ...
    kwargs = {}
    for key in qwen3_next_fields:
        if hasattr(hf_config, key):
            kwargs[key] = getattr(hf_config, key)

    # Defaults required by Qwen3NextDecoderLayer but absent from qwen3.5_moe.
    kwargs.setdefault("decoder_sparse_step", 1)
    kwargs.setdefault("mlp_only_layers", [])
    kwargs.setdefault(
        "intermediate_size",
        getattr(hf_config, "moe_intermediate_size", kwargs.get("hidden_size", 0) * 4),
    )

    new_config = Qwen3NextConfig(**kwargs)

    # Preserve extra fields that upstream MTP code may rely on.
    for key in (
        "n_predict",
        ...
        if hasattr(hf_config, key) and not hasattr(new_config, key):
            setattr(new_config, key, getattr(hf_config, key))

    new_config.model_type = "qwen3_next_mtp"
    n_predict = getattr(
        new_config, "num_nextn_predict_layers", None
    ) or getattr(new_config, "mtp_num_hidden_layers", None)
    new_config.n_predict = n_predict
    new_config.architectures = ["Qwen3NextMTP"]

    logger.info(
        "Rewrote %s draft config to Qwen3NextConfig for Ascend MTP",
        original_model_type,
    )
    return new_config
...
def _ascend_hf_config_override(hf_config):
    ...
    if hf_config.model_type in ("qwen3_5", "qwen3_5_moe"):
        hf_config = _rewrite_qwen3_5_config_for_mtp(hf_config)
    return hf_config
...
def patch_model_config_for_qwen3_mtp() -> None:
    ...
    def _ascend_model_config_post_init(self, *args, **kwargs):
        ...
        if self.runner == "draft":
            self.hf_config = _rewrite_qwen3_5_config_for_mtp(self.hf_config)
...
def patch_qwen3_5_mtp_post_init() -> None:
    ...
    def _ascend_qwen3_5_mtp_post_init(self, *args, **kwargs):
        _orig_post_init(self, *args, **kwargs)
        if getattr(self.hf_config, "model_type", None) == "qwen3_next_mtp":
            return
        ...
        if (
            ...
            and getattr(speculative_config, "_draft_hf_config", None) is not None
            and speculative_config._draft_hf_config.model_type == "qwen3_next_mtp"
        ):
...
def patch_qwen3_mtp_config_overrides() -> None:
    ...
    def _ascend_mtp_config_overrides(self, *args, **kwargs):
        speculative_config = getattr(self, "speculative_config", None)
        if (
            speculative_config is not None
            and getattr(speculative_config, "hf_config_override", None)
            and getattr(speculative_config, "_draft_hf_config", None) is not None
        ):
            override = speculative_config.hf_config_override
            for key, value in override.items():
                if hasattr(speculative_config._draft_hf_config, key):
                    setattr(speculative_config._draft_hf_config, key, value)
...
def patch_cudagraph_env_vars() -> None:
    """Honor VLLM_CUDAGRAPH_* env vars to limit NPU graph capture sizes.

    vLLM's ``--compilation-config`` is not always exposed in entrypoint
    scripts, so we provide environment variables as a lightweight alternative:

    - ``VLLM_CUDAGRAPH_CAPTURE_SIZES``: comma-separated list of batch sizes to
      capture (e.g. ``1,2,4,8,16,32,48,64,80,96``).
    - ``VLLM_CUDAGRAPH_MAX_CAPTURE_SIZE``: integer upper bound (optional).
    """
    import os

    cg_sizes_env = os.environ.get("VLLM_CUDAGRAPH_CAPTURE_SIZES")
    max_size_env = os.environ.get("VLLM_CUDAGRAPH_MAX_CAPTURE_SIZE")
    if not cg_sizes_env and not max_size_env:
        return

    try:
        from vllm.config.vllm import VllmConfig
    except ImportError:
        logger.debug("VllmConfig not available, skip cudagraph env patch")
        return

    _orig_post_init = VllmConfig.__post_init__

    def _ascend_cudagraph_env_post_init(self, *args, **kwargs):
        _cg_sizes_env = os.environ.get("VLLM_CUDAGRAPH_CAPTURE_SIZES")
        _max_size_env = os.environ.get("VLLM_CUDAGRAPH_MAX_CAPTURE_SIZE")
        if _cg_sizes_env:
            sizes = [
                int(x.strip())
                for x in _cg_sizes_env.split(",")
                if x.strip()
            ]
            self.compilation_config.cudagraph_capture_sizes = sizes
            self.compilation_config.max_cudagraph_capture_size = sizes[-1]
        if _max_size_env:
            self.compilation_config.max_cudagraph_capture_size = int(_max_size_env)
        _orig_post_init(self, *args, **kwargs)

    VllmConfig.__post_init__ = _ascend_cudagraph_env_post_init
    logger.info(
        "Patched VllmConfig.__post_init__ to honor VLLM_CUDAGRAPH_* env vars"
    )
