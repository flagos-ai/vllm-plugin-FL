...
def _per_token_group_quant_fp8_gcu(
    ...
    for g_id in range(pid, total_groups, NUM_SPC):
        ...
        # Offset calculations use int64: the byte offset is row*y_row_stride
        # (or g_id*group_size), which exceeds 2^31 for large prefill tensors
        # (e.g. 524288 rows x 7168 cols) and would wrap under int32, misdirecting
        # the y / y_q stores.  GCU honours int64 addressing regardless of
        # TORCH_GCU_ENABLE_INT64_AND_UINT64.  NB: .to() is applied only to the
        # computed index (row / row_g_id / g_id), never to the scalar kernel
        # args (y_row_stride / group_size), which torch's triton HOP analysis
        # binds as plain Python ints with no .to().
        y_ptr_offset = (
        ...
        y_q_cur = y_q_ptr + g_id.to(tl.int64) * group_size
...
def _per_token_group_quant_fp8_colmajor_gcu(
    ...
    for g_id in range(pid, total_groups, NUM_SPC):
        ...
        y_s_offset = (
            scale_col.to(tl.int64) * y_s_col_stride
            + scale_row.to(tl.int64)
        )
