...
vLLM 0.24.0 adaptations (FLA vendored + GDN package split):
- Patch target moved from GatedDeltaNetAttention._forward_core (base, no
  longer has the method) to QwenGatedDeltaNetAttention._forward_core in
  vllm.model_executor.layers.mamba.gdn.qwen_gdn_linear_attn.
- _warmup_prefill_kernels gained a v_dim arg -> call (mixed_qkv, 0).
...
def apply_ssm_patch():
    ...
    def _forward_core_kunlunxin(
        ...
        # v0.24.0: a non-spec batch mixing decode + prefill is split - the
        # decode front slice runs through the recurrent kernel and the prefill
        # tail through the chunk kernel, then outputs are stitched decode-first.
        split_non_spec = (
        ...
        if attn_metadata.num_prefills > 0 and mixed_qkv_non_spec is not None:
            ...
            if split_non_spec:
                ...
                prefill_slice = mixed_qkv_non_spec[num_decode_tokens:]
                a_prefill = a_non_spec[num_decode_tokens:]
                b_prefill = b_non_spec[num_decode_tokens:]
            else:
                prefill_slice = mixed_qkv_non_spec
        ...
        # 2.2: non-spec decode front slice (mixed decode+prefill batch only)
        if split_non_spec:
        ...
        # 2.3: non-spec prefill part (chunk kernel) or decode-only
        if attn_metadata.num_prefills > 0:
            ...
            _kunlunxin_write_ssm_cache(
                ssm_state, last_recurrent_state, prefill_state_indices
            )
            if split_non_spec:
                # Stitch decode-first to match the batch layout
                # (mixed_qkv_non_spec = [decode..., prefill...]).
