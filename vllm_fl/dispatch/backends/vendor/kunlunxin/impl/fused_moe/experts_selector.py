...
def vllm_topk_softmax(
    ...
    # The vendor fused op rejects some token/expert shapes used by vLLM 0.24
    # during profile and mixed-batch execution.  Keep the routing semantics in
    # the plugin with standard PyTorch ops instead of changing the vendor
    # runtime.  Compute softmax in FP32, as vLLM's fused reference does.
    scores = torch.softmax(gating_output.float(), dim=-1)
    weights, indices = torch.topk(
        scores, k=topk_weights.shape[-1], dim=-1, sorted=False
    )
    if renormalize:
    ...
    # vLLM 0.24's FL router does not consume this compatibility output, but
    # initialize it deterministically for callers that still retain the old
    # three-output ABI.
    token_expert_indices.copy_(
        ...
        )[:, None].expand_as(token_expert_indices)
    )
