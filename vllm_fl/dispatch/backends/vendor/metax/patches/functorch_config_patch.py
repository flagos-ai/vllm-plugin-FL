...
vLLM 0.19 patches ``torch._functorch.config.autograd_cache_normalize_inputs``
...
def _make_torch_config_entry(name: str, default: bool) -> Any | None:
    ...
    try:
        # PyTorch main passes the key name to _ConfigEntry.
        return config_entry_cls(config, name)
    except TypeError:
        # PyTorch 2.8 only accepts the config object.
