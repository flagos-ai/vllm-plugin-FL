    ...
    __gm__ T *src, __gm__ T *dst, int64_t T_len)
{
    ...
    // Head count padded up so that HA * sizeof(T) is 32B-aligned; the
    // padding columns are zero-filled before the transpose.
    constexpr int32_t HA = ((H * ES + 31) / 32) * 32 / ES;
    constexpr int32_t SRC_UB = 0;
    constexpr int32_t DST_UB = SRC_UB + BLOCK * HA * ES;
    constexpr int32_t TMP_UB = DST_UB + HA * BLOCK * ES;

    using UBSrcFull = Tile<TileType::Vec, T, BLOCK, HA, BLayout::RowMajor,
                           BLOCK, HA, SLayout::NoneBox, 512, PadValue::Zero>;
    using UBSrcDyn  = Tile<TileType::Vec, T, BLOCK, HA, BLayout::RowMajor,
                           DYNAMIC, DYNAMIC, SLayout::NoneBox, 512, PadValue::Zero>;
    using UBDst     = Tile<TileType::Vec, T, HA, BLOCK, BLayout::RowMajor,
                           HA, BLOCK, SLayout::NoneBox, 512>;
    using UBDstDyn  = Tile<TileType::Vec, T, H, BLOCK, BLayout::RowMajor,
                           DYNAMIC, DYNAMIC, SLayout::NoneBox, 512>;
    using UBTmp     = Tile<TileType::Vec, T, BLOCK, HA, BLayout::RowMajor,
                           BLOCK, HA, SLayout::NoneBox, 512>;
    ...
    for (int64_t bi = static_cast<int64_t>(cid); bi < num_tok_blocks;
...

        {
            ...
            TASSIGN(ld, SRC_UB);
            TLOAD(ld, gm);
            if (valid != BLOCK || HA != H) TFILLPAD_INPLACE(ub_src, ld);
        }
