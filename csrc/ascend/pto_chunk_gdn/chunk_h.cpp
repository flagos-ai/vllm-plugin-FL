    ...
    uint64_t ffts_addr)
{
  ...
  for (int64_t wi = 0; wi < (total_work + block_num - 1) / block_num; ++wi) {
    ...
    // Drain the fs TSTORE before the next work item reuses S_UB_HALF;
    // otherwise the MTE3 read of the UB races with the zeroing/TCVT at the
    // start of the next item and corrupts the stored final state (only the
    // last work item per core was previously correct).
