...
def test_prefill_attention(dtype, num_heads, num_kv_heads, head_size, block_size):
    ...
    block_table = torch.zeros(batch_size, max(num_blocks_per_seq), dtype=torch.int32, device=device)
    ...
    for i, seq_len in enumerate(seq_lens):
        ...
        slot_mapping[token_offset : token_offset+seq_len] = torch.tensor(current_seq_slots, device=device)
        ...
        ref_q_list.append(torch.nn.functional.pad(q_slice, (0,0,0,0,0, max_seq_len - seq_len)))
        ref_k_list.append(torch.nn.functional.pad(k_slice, (0,0,0,0,0, max_seq_len - seq_len)))
        ref_v_list.append(torch.nn.functional.pad(v_slice, (0,0,0,0,0, max_seq_len - seq_len)))
    ...
    kv_cache = create_kv_cache(total_blocks, block_size, num_kv_heads, head_size, dtype, device)
    
    impl = AttentionFLImpl(num_heads, head_size, scale, num_kv_heads, None, None, "auto")
...
def test_decode_attention(dtype, num_heads, head_size):
    ...
    query = torch.randn(batch_size, num_heads, head_size, dtype=dtype, device=device) # [B, H, D] flattened
    curr_k = torch.randn(batch_size, num_kv_heads, head_size, dtype=dtype, device=device)
    curr_v = torch.randn(batch_size, num_kv_heads, head_size, dtype=dtype, device=device)
    ...
    # 3. Run Impl
    seq_lens_tensor = torch.tensor([pl + 1 for pl in past_lens], dtype=torch.int32, device=device)
    ...
    output_buffer = torch.zeros(batch_size, num_heads, head_size, dtype=dtype, device=device)
    ...
    # 5. Compare
    assert torch.allclose(output_buffer, ref_out_flat, atol=1e-3, rtol=1e-3), \
