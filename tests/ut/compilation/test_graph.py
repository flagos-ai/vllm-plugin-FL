...
try:
    from vllm_fl.compilation.graph import GraphWrapper, GraphEntry
except ImportError:
    pass
...
class MockEnumMember:
    ...
    def __eq__(self, other):
        ...
        if hasattr(other, 'name'): return self.name == other.name
        return False
    def __str__(self): return self.name
    def __repr__(self): return f"MockMode.{self.name}"
...
def mock_dependencies():
    with patch("vllm_fl.compilation.graph.envs") as mock_envs, \
...
def test_cuda_graph_capture_and_replay(mock_dependencies, mock_config):
    ...
    torch.testing.assert_close(output2, expected_output_2, rtol=1e-3, atol=1e-3, msg="Replay output mismatch")
