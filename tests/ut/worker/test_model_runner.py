...
class MockSchedulerOutput:
    def __init__(self, **kwargs):
        ...
        self.scheduled_cached_reqs = kwargs.get(
            "scheduled_cached_reqs", MagicMock(req_ids=[])
        )
        ...
        self.scheduled_spec_decode_tokens = kwargs.get(
...
class TestModelRunnerFunctions:
    ...
    def test_update_states_logic(self, mock_get_pp_group, mock_runner):
        ...
        scheduler_output = MockSchedulerOutput(
            scheduled_new_reqs=[new_req], num_scheduled_tokens={"req_1": 3}
        )
    ...
    def test_calc_spec_decode_metadata(self, mock_runner):
        ...
        mock_runner.input_ids.gpu = torch.zeros(
            10, dtype=torch.int32, device=mock_runner.device
        )

        metadata = mock_runner._calc_spec_decode_metadata(
    ...
    def test_prepare_input_ids_with_async_cache(self, mock_runner):
        ...
        mock_runner.input_ids = MagicMock()
        mock_runner.input_ids.gpu = torch.zeros(
            2, dtype=torch.int32, device=mock_runner.device
        )
    ...
    def test_prepare_inputs_positions_calculation(self, mock_runner):
        ...
        mock_runner.input_batch.token_ids_cpu_tensor = torch.zeros(
            200, dtype=torch.int32, device="cpu"
        )

        class MockBuf:
            def __init__(self, size, dtype):
                self.np = np.zeros(
                    size, dtype=np.int64 if dtype == torch.int64 else np.int32
                )
        ...
        scheduler_output = MockSchedulerOutput(
            total_num_scheduled_tokens=5, num_scheduled_tokens={"r1": 2, "r2": 3}
        )

        mock_runner.requests = {
            "r1": MagicMock(num_tokens=12),
            "r2": MagicMock(num_tokens=3),
        }
        mock_runner.get_local_padding = MagicMock(return_value=0)

        with patch(
            "vllm_fl.worker.model_runner.ubatch_split", return_value=(None, None)
        ):
