...
The recurrent kernel takes beta in the activation dtype while g stays float32.
A hard-coded bfloat16 cast makes fp16 decode silently wrong, and the sigmoid
...
def test_gating_shapes_and_dtypes(dtype):
    a, b, a_log, dt_bias = _inputs(dtype)

    g, beta = _kunlunxin_decode_gating(a, b, a_log, dt_bias)
...
def test_beta_is_sigmoid_b_fp32(dtype):
    ...
    _, beta = _kunlunxin_decode_gating(a, b, a_log, dt_bias)
...
def test_g_matches_fp32_reference(dtype):
    ...
    g, _ = _kunlunxin_decode_gating(a, b, a_log, dt_bias)
